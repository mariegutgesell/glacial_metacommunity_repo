...
for(n in 1:50) {
  current_xtabs_list <- fish[[paste0("Iteration_", n)]]
...
write.csv(tb_results_all, "data/temporal_betadiv_results/fish_temporal_betadiv_results_bootstrapped.csv")
...
##Plot out 
fish_mcv <- ggplot(tb_results_all_summary, aes(x = combo_type, y = GammaHBD_mean, fill = taxa, group = combo_type)) +
...
fish_mcs <- ggplot(tb_results_all_summary, aes(x = combo_type, y = PhiHBD_mean, fill = taxa, group = combo_type)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
fish_lcv <- ggplot(tb_results_all_summary, aes(x = combo_type, y = AlphaHBD_mean, fill = taxa, group = combo_type)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
library(ggpubr)
fish_comp_fig <- ggarrange(fish_mcv,fish_lcv, fish_mcs, legend = "none", 
                             labels = c("c)", "d)", "e)"),
...
selected_inverts <- fish[select_iterations1]
...
fish_df <- final_df %>%
  ...
  mutate(taxa = "fish") %>%
  group_by(taxa, Combo, combo_type, streamID_2, Iteration, Month_Year, species2) %>%
  ...
  select(taxa, Combo, combo_type, streamID_2, Month_Year, rep, species2, iteration_total_biomass)
...
# 1. Heterogeneous
df_het <- fish_df %>% filter(Combo == "Glacier-fed_Rain-fed_Snow-fed")
pD_het <- plot_nmds_trajectory(df_het, c("darksalmon", "lightpink", "azure3"), "a)")
pD_het
# 2. 2:1 Snow-dominated
df_21 <- fish_df %>% filter(Combo == "Rain-fed_Snow-fed_Snow-fed")
pD_21 <- plot_nmds_trajectory(df_21, c("lightpink", "azure3", "azure3"), "b)")
pD_21
# 3. Homogeneous
df_hom <- fish_df %>% filter(Combo == "Rain-fed_Rain-fed_Rain-fed")
