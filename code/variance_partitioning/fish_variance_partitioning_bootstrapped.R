##Partition Metacommunity Variation -- Bootstrapped fish data


source("code/sp_matrix_arrays/fish_sp_matrix_bootstrapped.R")

rm(list = ls()[!ls() %in% c("fish_xtabs_list_collection")])
...
for(n in 1:50) {
  current_xtabs_list <- fish_xtabs_list_collection[[paste0("Iteration_", n)]]
...
write.csv(partition_results_all, "data/variance_partitioning_results/fish_var_partition_results_bootstrapped.csv")

partition_results_all$combo_type <- ordered(partition_results_all$combo_type, 
                                            levels = c("Homogenous", "2:1", "Heterogenous"))
##Metacommunity stability
meta_cv <- ggplot(partition_results_all, aes(x = combo_type, y = CV_C_R, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  #  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_standard_error, ymax = CV_C_R_mean + CV_C_R_standard_error), position = position_dodge(width = 0.9), width = 0.25)+
  theme_classic() +
...
##invert mean 
meta_mean <- ggplot(partition_results_all, aes(x = combo_type, y = mean_metacom, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  #  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_standard_error, ymax = CV_C_R_mean + CV_C_R_standard_error), position = position_dodge(width = 0.9), width = 0.25)+
  theme_classic() +
...
meta_sd <- ggplot(partition_results_all, aes(x = combo_type, y = sd_metacom, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  #  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_standard_error, ymax = CV_C_R_mean + CV_C_R_standard_error), position = position_dodge(width = 0.9), width = 0.25)+
  theme_classic() +
...
##Local Community stability
local_cv <- ggplot(partition_results_all, aes(x = combo_type, y = CV_C_L, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  #  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_standard_error, ymax = CV_C_R_mean + CV_C_R_standard_error), position = position_dodge(width = 0.9), width = 0.25)+
  theme_classic() +
...
cv_diff <- ggplot(partition_results_all, aes(x = combo_type, y = cv_diff, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  #  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_standard_error, ymax = CV_C_R_mean + CV_C_R_standard_error), position = position_dodge(width = 0.9), width = 0.25)+
  theme_classic() +
...
##Asynchrony -- community asynchrony local to regional -- driven by spatial community dissimilarity 
##Is the spatial synchrony of total community biomass among local patches
comm_async <- ggplot(partition_results_all, aes(x = combo_type, y = phi_C_L2R, group = combo_type, fill = combo_type)) +
  geom_boxplot()+
  theme_classic() +
...
fish_fig <- ggarrange(meta_cv, local_cv,cv_diff, comm_async, legend = "none", 
                        labels = c("a)", "b)", "c)", "d)"),
