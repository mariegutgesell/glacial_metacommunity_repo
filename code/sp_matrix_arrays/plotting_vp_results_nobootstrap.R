#### Plotting Variance Paritioning Results across all taxa -- non bootstrapped results
...
inverts <- read.csv("data/variance_partitioning_results/invert_var_partition_results.csv") %>%
#  rename(Combination = "Combo") %>%
  select(taxa, Combination, combo_type, CV_C_L, CV_C_R, phi_C_L2R, cv_diff, mean_metacom, sd_metacom)
...
df_4 <- df_2 %>%
  ...
  select(taxa, Combination, combo_type_ab, CV_C_R, CV_C_L, cv_diff, phi_C_L2R, mean_metacom, sd_metacom, CV_reduction_factor) %>%
...
meta_cv_2 <- ggplot(df_4, aes(x = combo_type_ab, y = CV_C_R_mean, group = combo_type_ab, fill = taxa)) +
...
local_cv_2 <- ggplot(df_4, aes(x = combo_type_ab, y = CV_C_L_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
cv_diff_2<- ggplot(df_4, aes(x = combo_type_ab, y = cv_diff_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
comm_async_2<- ggplot(df_4, aes(x = combo_type_ab, y = phi_C_L2R_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
CV_rf<- ggplot(df_4, aes(x = combo_type_ab, y = CV_reduction_factor_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
...
df_3 <- df_2 %>%
  filter(Combination %in% c("Glacier-fed_Rain-fed_Snow-fed", "Rain-fed_Rain-fed_Rain-fed", "Rain-fed_Snow-fed_Snow-fed")) %>%
  select(taxa, Combination, combo_type_ab, CV_C_R, CV_C_L, cv_diff, phi_C_L2R) %>%
...
meta_cv_2 <- ggplot(df_3, aes(x = combo_type_ab, y = CV_C_R_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
  geom_errorbar(aes(ymin = CV_C_R_mean - CV_C_R_sd, ymax = CV_C_R_mean + CV_C_R_sd, group = taxa), position = position_dodge(width = 0.9), width = 0.25) +
  scale_fill_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
  scale_colour_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
...
local_cv_2 <- ggplot(df_3, aes(x = combo_type_ab, y = CV_C_L_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
  geom_errorbar(aes(ymin = CV_C_L_mean - CV_C_L_sd, ymax = CV_C_L_mean + CV_C_L_sd, group = taxa), position = position_dodge(width = 0.9), width = 0.25) +
  scale_fill_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
  scale_colour_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
...
cv_diff_2<- ggplot(df_3, aes(x = combo_type_ab, y = cv_diff_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
  geom_errorbar(aes(ymin = cv_diff_mean - cv_diff_sd, ymax = cv_diff_mean + cv_diff_sd, group = taxa), position = position_dodge(width = 0.9), width = 0.25) +
  theme_classic() +
  scale_fill_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
...
comm_async_2<- ggplot(df_3, aes(x = combo_type_ab, y = phi_C_L2R_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
  geom_errorbar(aes(ymin = phi_C_L2R_mean - phi_C_L2R_sd, ymax = phi_C_L2R_mean + phi_C_L2R_sd, group = taxa), position = position_dodge(width = 0.9), width = 0.25) +
  theme_classic() +
  scale_fill_manual(values = c("darkgreen",  "deepskyblue", "darksalmon"))+
...
cv_diff_peri <- df_3 %>%
  filter(taxa == "Periphyton") %>% 
  ggplot(aes(x = combo_type_ab, y = cv_diff_mean, group = combo_type_ab, fill = taxa)) +
  geom_col(aes(color = taxa), alpha = 0.5, linewidth =1) +
  geom_errorbar(aes(ymin = cv_diff_mean - cv_diff_sd, ymax = cv_diff_mean + cv_diff_sd, group = taxa), position = position_dodge(width = 0.9), width = 0.25) +
  theme_classic() +
